...
function reticleAngularRadius(value, screenReticleRadius) {
    return value == RETICLE_SIFO ? screenReticleRadius * 70.0 / 44.0 : screenReticleRadius;
}

function reticleValidPoleDistance(value, distanceArcmin) {
    return ioptronFinite(distanceArcmin) && distanceArcmin >= 0.0
...
function ioptronMarkerPosition(
    output,
    fullHourAngle,
    elapsedSeconds,
    poleDistanceArcmin,
    ...
    if (
        output == null || output.size() < 2 || !ioptronFinite(elapsedSeconds)
            || !ioptronFinite(centerX)
            || !ioptronFinite(centerY)
            || !ioptronFinite(r70)
            || r70 <= 0.0
            || !ioptronFinite(maximumPoleDistance)
            || maximumPoleDistance < 0.0
            || !ioptronFinite(poleDistanceArcmin)
            || poleDistanceArcmin < 0.0
            || poleDistanceArcmin > maximumPoleDistance
    ) {
        return false;
    }
