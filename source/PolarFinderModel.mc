...
class PolarFinderModel {
  ...
  var confirmed=false; var formatDms=false; var reticleType=RETICLE_GENERIC; var pressureMode=0; var manualPressure=1013.25; var manualPressureSet=false;
  ...
  function load() {
    ...
    formatDms=valueOr(Application.Storage.getValue("formatDms"),false); reticleType=normalizeReticleType(Application.Storage.getValue("reticleType")); pressureMode=valueOr(Application.Storage.getValue("pressureMode"),0).toNumber();if(pressureMode<0||pressureMode>2){pressureMode=0;}
  ...
  function savePreferences(){Application.Storage.setValue("formatDms",formatDms);Application.Storage.setValue("reticleType",reticleType);Application.Storage.setValue("pressureMode",pressureMode);Application.Storage.setValue("manualPressure",manualPressure);Application.Storage.setValue("manualPressureSet",manualPressureSet);Application.Storage.setValue("temperature",temperature);Application.Storage.setValue("humidity",humidity);Application.Storage.setValue("calculationAlert",calculationAlert);Application.Storage.setValue("firstUse",firstUse);}
